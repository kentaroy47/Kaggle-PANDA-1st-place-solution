...
def split_tile_nms(
    ...
    assert len(boxes_selected) == n_tile, "Invalid boxes size !"
...
def split_tile_nms_resize(
    ...
    tiles = crop_imgmask_tile(img, mask=None, coords=boxes_selected)
    return tiles
...
def make_parse():
    ...
    arg("--stride-rate", type=int, default=2)
...
def main():
    ...
    out_dir = (
        root
        / f"numtile-{num_cluster}-tilesize-{tile_size}-nms-striderate-{stride_rate}"
    )
    ...
    with zipfile.ZipFile(out_train_zip, "w") as img_out:
        for img_id in tqdm(train.image_id):
            ...
            if (h * w) < (2000 * 2000):
                ...
                print(f"{img_id}, re read: {img.shape}")
                ts = tile_size * 2
            ...
            try:
                ...
                tiles = get_tiles(img, tile_size, num_cluster, mode=0)
